...
from config.app_logging import logger as app_logger
...
def handle_suggest(ack, respond, command, logger):
    ...
    try:
        ...
        app_logger.info(f"Got {len(existing)} existing appointments")
        
        suggestions = suggest_appointments(req, existing)
        app_logger.info(f"Suggester returned {len(suggestions)} suggestions")
        app_logger.info(f"First suggestion: {suggestions[0] if suggestions else 'None'}")
        
        blocks = format_blocks(suggestions, address, time_windows["time_windows"])
        app_logger.info(f"Formatted {len(blocks)} Slack blocks")
